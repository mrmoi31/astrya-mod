...
import net.mcreator.astrya.item.inventory.IronBackpackItemInventoryCapability;
...
public class IronBackpackItemItem extends Item {
	public IronBackpackItemItem() {
		super(new Item.Properties().tab(AstryaModTabs.TAB_ASTRYA).stacksTo(1).rarity(Rarity.COMMON));
	}
		...
		if (entity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openGui(serverPlayer, new MenuProvider() {
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					return new Test2Menu(id, inventory, packetBuffer);
				}
			}, buf -> {
	...
	public ICapabilityProvider initCapabilities(ItemStack stack, @Nullable CompoundTag compound) {
		return new IronBackpackItemInventoryCapability();
	}
